...
## 2026-09-11 (continuation) — `HypotheticalControlSchedule` + `agent_interface.py`: predict/act/observe, hackathon prep foundation

**Hackathon context:** CoreWeave/AGI House event, ~2 days out. Plan (per the compliance discussion earlier this session, and Christian's own established pattern from the prior AGI House event): pre-build the non-AI foundation, build the actual self-correcting agent loop fresh at the event. This entry is foundation only -- no decision/reasoning logic, by design.

**`HypotheticalControlSchedule` (in `forward_sim.py`, right after `class ControlSchedule`).** The one real capability gap identified this session: everything forward_sim.py could do was REPLAY a real flight's recorded inputs (`test_against_run`'s job) -- there was no way to ask "if I commanded X, what would happen?" from an arbitrary state. Turned out `forward_simulate()` already duck-types its `control_schedule` parameter against `.throttle(t)`/`.turn_axis(t)`/`.directional_axis(t)`/`.real_aero(t)` -- no changes needed there at all. `HypotheticalControlSchedule` implements that same interface against a caller-supplied list of `{t, throttle, turn_axis, directional_axis}` waypoints (step-function held between them, matching `ControlSchedule`'s own documented reasoning for why real control inputs shouldn't be interpolated), with per-field carry-forward so a caller can write e.g. `{"t": 5.0, "turn_axis": 0.5}` without repeating throttle.

**Verified in a sandbox** (not just reasoned about): fed a full-throttle-then-cutoff hypothetical plan (`throttle=1` at t=0, `throttle=0` at t=20) into `forward_simulate` from a real observed starting state -- velocity climbed steadily to 558 m/s through t=20, then immediately started falling (551 -> 512 -> 462 m/s) the instant the cutoff waypoint took effect. Clean, correct step-function response at exactly the right boundary.

**`analysis/agent_interface.py` (new file) -- the predict/act/observe boundary.** Three functions:
- `observe()` / `observe_to_state()` -- wraps `telemetrysnapshot` with a fixed agent-useful field list, translates it into the shape `predict()` needs. One place the field-name translation lives, not re-derived per caller.
- `act(command, allow_full_authority=False)` -- sends a command through the same file-protocol `sfsprobe/probe_cmd.py` uses (extracted into an importable function, not just a CLI script), with safety clamps on `turn`/`throttle` specifically: `MAX_TURN_AXIS_MAGNITUDE=0.5`, `MAX_THROTTLE_STEP=0.3` per call. **Direct response to today's two live-testing incidents** (full throttle+full turn triggered a real launch and an unresponsive probe; even `throttle=0.3`+full turn instantly crashed the rocket) -- see `bookkeeping/active_state.md`'s incident note. Every other command passes through unmodified; this is not a general validator.
- `predict(state, waypoints, duration_s, ...)` -- forward-simulates a hypothetical plan from any state via `HypotheticalControlSchedule`. Regime-aware by default: auto-enables the AoA=0 gate whenever starting height is inside the atmosphere (matching the 2026-09-09 finding that ungated atmospheric prediction is unusable, not just suboptimal), and returns a `confidence` label grounded in this project's own measured accuracy -- `"high"` for vacuum (any maneuver) or atmosphere-with-no-turn (confirmed ~0.05%), `"low"` for atmosphere-with-turn (the deprioritized, inherently unstable regime). Not a calibrated number, deliberately -- calibration is separate, unstarted work.

**Live-verified end to end** (self-test block, `python3 agent_interface.py`): `observe()` pulled real telemetry off the actual live rocket (279.5t, matches the reverted 3-stage craft), `observe_to_state()` translated it, `predict()` ran a genuine 5s blind forward simulation from that real observed state and returned `confidence=high, collided=False`. No replay file, no MCP round-trip -- straight from live game state through the physics engine.

**One pre-existing nuance surfaced, not a new bug:** the 5s no-op prediction ended at `h=-55.9` (below "ground") -- `forward_simulate`'s terrain handling needs a real `terrain_lookup` callback to stop at actual pad elevation; without one it falls through a flat sea-level assumption. Existing behavior, unrelated to today's work -- flagged for whoever wires up a terrain-aware `predict()` call during the hackathon (probe already has `terrain`/`terraingeo` commands that could supply the lookup).

**Explicitly NOT built here, by design:** no divergence detection, no correction logic, no goal-seeking, no calibrated confidence numbers -- those are the actual hackathon-scoped work.

**Easy-Autonomous cleanup item, not a research finding.** `_prepare_replay` and `load_control_schedule` both hard-required the underscore/dotted key names (`output_TurnAxisTorque`, `output_DirectionalAxis.x/y`, `location.position.x/y`, `rb2d.mass/rotation/angularVelocity`) that only `analysis/convert_flight_compat_keys.py`'s output ever had — raw archived `.gz` flights (camelCase: `outputTurnAxisTorque`, `directionalAxisX/Y`, `px/py`, `m`, `rot`, `angv`) failed loudly with a clear error, requiring a separate manual conversion pass every time (see the 2026-09-09 entry above for the discovery).

Added `_normalize_camelcase_telemetry(rows)` — promotes `convert_flight_compat_keys.py`'s exact mapping into the module itself, mutating each row in place to add the underscore keys from their camelCase equivalents whenever the underscore key isn't already present. A no-op on files that already have the underscore keys (checked via `rows[0]` before doing any work), so `_compat.jsonl` files and the standalone script both remain valid — this just makes the conversion step optional. Wired into both `_prepare_replay` and `load_control_schedule`, right after `st.load_samples()`, before the existing validation check (which now passes automatically once the camelCase keys are normalized).

**Verified, not just reasoned about:** ran `test_against_run` on `long_coast_fuel_exhaust_turn_2026-09-08`'s raw decompressed `.gz` (never touched by the standalone conversion script) and got byte-identical output to the same call against the pre-converted `_compat.jsonl` — 0.054% h-error, 77.27m position offset at 30s, both paths. Confirms the normalization is both correct and safely idempotent.

---

## 2026-09-09 — merged the AoA=0 regime-gated fix from `forward_sim_v2.py` into `forward_sim.py`

**Straight merge, no logic changes.** The H8/H9/H10 fix built and tested 2026-09-08 (see that entry below) lived only in the scratch `forward_sim_v2.py` copy. Diffed the two files, confirmed the change was exactly three self-contained hunks with no other drift between the two copies, and applied all three to `forward_sim.py`:

1. Two new `DEFAULT_FLAGS` entries: `aero_torque_aoa_gate_deg` (None by default — disabled, matches prior behavior exactly) and `aero_torque_aoa_gate_cooldown_s` (0.0 default).
2. The gate computation in `_derivative` — reuses `forward_sim.py`'s own existing `_wrap180` helper (already present, no duplicate needed) to get AoA from `(theta, vx, vy)`, then the H9 turn-awareness guard (`_current_turn_axis` below 0.1) and H10 cooldown timer (`_sim_t_now` / `_last_turn_t`) before allowing the gate to fire.
3. `aoa_gate_ok` added as a third condition (alongside the existing `flags["aero_torque"]` and `cop_x is not None` checks) on the torque-accumulation line.
4. Two bookkeeping writes (`flags["_current_turn_axis"]`, `flags["_sim_t_now"]`) added right after `turn_axis` is computed each step, so the gate has current-step data to read the *next* step.

**Regime-check design note (this was the real open item, not just "paste the diff"):** the gate does NOT rely on a human picking a turn-free window by hand — that was last session's stopgap for testing, not what shipped. In the merged code the regime check is fully automatic and per-step: `_current_turn_axis` and `_sim_t_now` are written by the main integration loop itself every step, so the gate's turn-awareness and cooldown guards read real live state from the same sim run, not an externally-supplied window boundary. Still off by default (`aero_torque_aoa_gate_deg=None`) until a caller opts in.

**Live-verified same day (continuation session, below), 2026-09-09.** Short version: the merge itself is clean, but a caller MUST turn the gate on for atmospheric predictions to be usable at all -- see that entry for the full story and numbers.

---

## 2026-09-09 (continuation) — live-verified the merge, found the atmospheric-coast "achieved" numbers require the gate ON (not default flags), two real telemetry-schema gotchas

**Goal: confirm the historic "0.497% at 30s" atmospheric-coast number still holds post-merge.** Picked `long_coast_fuel_exhaust_turn_2026-09-08` (the flight that number came from) and re-ran `test_against_run`/`test_against_run_trajectory` against the merged `forward_sim.py`. Took a real debugging detour to get there — full trail below, since both gotchas will bite again otherwise.

**Gotcha 1 — raw archived telemetry uses camelCase, not what the loader expects.** `_prepare_replay` hard-requires literal keys `output_TurnAxisTorque` / `output_DirectionalAxis.x/y` in row 0. Confirmed by decompressing the raw `.gz` directly (not just trusting the MCP tool's display) that this project's actual recordings use `outputTurnAxisTorque` / `directionalAxisX` / `directionalAxisY` — no underscore/dotted keys at all. A `convert_flight_compat_keys.py` script and pre-generated `_compat.jsonl` files already exist in `analysis/` for exactly this reason; the fix is just remembering to use them. Filed in `active_state.md`'s Monitored hooks so this isn't rediscovered painfully again.

**Gotcha 2 — `throttleOut` (nested AND top-level) reads 0 even mid-burn.** Confirmed directly: mass visibly dropping, velocity past 1500 m/s, `engines[0].throttleOut` and the compat-added top-level `throttleOut` both still 0 at that sample. This isn't the already-fixed 2026-09-07 field-lookup bug (that fix correctly finds and returns whatever's there) — the field itself is genuinely unpopulated/broken in this recording. `thr` (the 0-1 commanded value) is correct throughout and is the field to use.

**The real finding, after both gotchas were out of the way:** with `throttle_field="thr"` confirmed correct (traced `ControlSchedule.throttle()` directly — returns 1.0 every tick during the real burn) but flags left at default, the prediction still cratered: predicted altitude went NEGATIVE by t=30s while the real craft was at 15.6km/1206 m/s. Looked exactly like a zero-thrust bug at first (matching the 09-07 pattern), but tracing `rotation_error_deg` per-step showed the real cause: **175 degrees of rotation error within the first ONE second of simulation**, on a real flight with zero commanded turns the whole way. Isolated with a flags A/B test: `aero_torque: False` alone drops the error to exactly 0.0 every step; `sas: False` alone changes nothing. This is the same AoA~0 table defect from 2026-09-08's H8 finding, just more severe in practice than the "~-85 to -92 deg/s^2 spurious torque" framing suggested — sustained, it flips the craft within a second and the resulting mispointed thrust nosedives it into the ground, which is what looked like "zero thrust" from the outside.

**Turning the gate ON (`aero_torque_aoa_gate_deg`, tested 1/2/5 — all three gave byte-identical results) fixes it and reproduces the historic numbers almost exactly**, computed fresh today: 0.054% h-error / 10.9m position offset at 20s, 0.054% / 77.3m at 30s, 0.056% / 146.8m at 34s. Matches the 09-08 session's 0.497%/0.671% claims (small differences are consistent with normal float/version noise, not a discrepancy).

**Correction this forces to how the merge should be read:** "gate defaults off, matches prior behavior" is true and confirmed, but "prior behavior" for ANY atmospheric window — coast included, not just turns — is this badly broken. The 30s-ahead coast/vacuum "achieved" claim was always implicitly gate-ON; it was never a default-flags result. Whether the gate should now default ON for atmospheric predictions (rather than requiring every caller to remember it) is an open design question, not yet decided — see `active_state.md`'s Immediate next experiment.

**Method note:** debugged by copying `forward_sim.py` + its dependencies (`aoa_dragarea.py`, `sfs_telemetry.py`) into a sandbox and calling `fsim.test_against_run`/`test_against_run_trajectory` directly with a monkeypatched trace on `ControlSchedule.throttle()`, rather than guessing through the MCP tool as a black box — confirmed the MCP wrapper (`sfsprobe_mcp/server.py`) forwards `throttle_field` correctly (no bug there), and let the actual per-step rotation error surface directly instead of only the aggregate final-point error.

---

## 2026-09-08 — scripted matched-pair flights, root-caused the AoA=0 table defect and fixed it (regime-gated), 30s-ahead goal loop (coast/vacuum solved, atmospheric-turn isolated as the real remaining gap), SOI/PhysicsMode research, staging mod feature

**Four real flights recorded and analyzed this session**, all using the throttle-field-fixed / index-based-ControlSchedule pipeline from 2026-09-07:
- `atmosphere_scripted_turns_2026-09-08` — 3 spaced turn events (turn+1 x1.5s, turn-1 x1.5s, turn+1 x3s), scripted (not live button-mashing), to cleanly measure reliable-horizon-vs-starting-rotation-rate.
- `direction_sign_matched_pair_2026-09-08_A2`/`_B` — a matched pair (fresh launch, identical ascent, single turn differing only in sign: +1 vs -1) built specifically to isolate turn-SIGN as a variable. Confirmed via real telemetry the two ascents track within noise (sub-1% alt/vel, thousandths of a degree rotation) up to the turn. Original Flight A was lost once to the archive-overwrite-before-tagging gotcha (again) and had to be redone as A2 — tag immediately, before any other `telemetry on/off`, no exceptions.
- `long_coast_fuel_exhaust_turn_2026-09-08` — 163s flight: powered coast to a NATURAL (unscripted) fuel-exhaustion cutoff (~t=36.5s, not commanded), ~78s of real unpowered vacuum coast, then a turn at t~107s/h~116,584m (well above the 30km atmosphere ceiling, i.e. a vacuum turn).

**Root-caused the turn-divergence "sign asymmetry" from the matched pair — it wasn't sign, it was a table defect at AoA~0.** The matched pair showed B (-1, smaller real starting rate 46.3 deg/s) diverging WORSE than A2 (+1, larger rate 63.7 deg/s) — backwards from magnitude-driven sensitivity, and only A2 got fixed by finer dt (1/60), not B. Traced with a direct per-tick real-vs-table-predicted alpha comparison (`_aero_torque_diagnostic` against real `comX/comY/rbInertia` at each real recorded state, no compounding trajectory involved): the static AoA table predicts large SPURIOUS torque (~-85 to -92 deg/s^2) exactly at AoA~0, where real torque is genuinely ~0. Confirmed this isn't a general rate-dependence gap (decile-sorted by |omega|, median residual doesn't trend with rate at all — the earlier Pearson correlation was an artifact of a few AoA~0 outliers dominating the mean, not the median) and isn't an aero-table asymmetry either (position-independent lever arms at mirrored AoA, e.g. +-5/10/20 deg, are genuine mirror images of each other — checked directly).

**Fix, implemented and tested on `analysis/forward_sim_v2.py` (NOT yet merged into `forward_sim.py` — see Immediate next in active_state.md):** new `aero_torque_aoa_gate_deg` flag zeroes only the aero TORQUE contribution (not the translational drag force, which is fine near AoA=0) when |AoA| is below the threshold. Robust across thresholds 0.5-5 deg. **Regime-dependent result, the real finding:** fixes pure-coast windows completely (99.97% → 0.040% position error) but made turn-inclusive windows WORSE at every threshold and every cooldown duration tested up to 100s (97-103% vs 24.8% ungated) — once a turn has happened anywhere in a window's history, the system is already in a compounding-sensitivity regime and removing even a locally-correct simplification perturbs it further. The gate is only safe in windows with ZERO turn commands anywhere in their history, not "far enough" from one — confirmed by testing, not assumed.

**30s-ahead goal-loop (target: <1% error at 30s), run against the <1% error / exhaust-existing-data rules Christian set:**
- **Coast regime: goal genuinely achieved, strong evidence.** Powered atmospheric coast: 0.497% at exactly 30s, 0.671% at 34s (data ceiling, error growing smoothly not exponentially). Spanning the NATURAL fuel-exhaustion cutoff: 0.06-0.14% crossing the transition (validates the throttle-replay design generalizes to an unscripted cutoff, not just commanded `throttle 0`). Unpowered vacuum coast: ~0.001% even at 60-70s.
- **Vacuum turn: also essentially solved** (0.001% position, single-digit-deg rotation error) — confirmed via the long-coast flight's t~107s turn at h=116,584m, `aeroTorque:0` in real telemetry. This reframed the whole "turns are chaotic" conclusion: it was never turning itself, it's specifically turning THROUGH DENSE ATMOSPHERE, where the imperfect aero-torque model dominates.
- **Atmospheric turn: still the one open gap**, now much more precisely diagnosed than "turns are chaotic" — ruled out sign, dt, integrator (symplectic_euler), sas_order, box2d_rotation_clamp, live_inertia, and the AoA=0 fix itself (all neutral-to-negative). Remaining credible lever needs new data (a `dragareasweep` captured while the craft is actively spinning at known real rates, not just static angles) — logged as a proper End-of-Goal-Table "needs new data" entry, not a dead end.
- Harness: `analysis/eval_harness.py` (position-error-normalized-by-real-displacement metric, explicitly defined since "1% error" was ambiguous per the goal prompt's own rule) plus `analysis/rate_dependence_check.py` (the per-tick real-vs-table residual/rate-correlation check that found the AoA=0 defect).

**SOI / rails-mode research (for the planned past-SOI flight, not yet flown):** confirmed via IL (`sfs_source_reference.md` §D4/B5) Earth's SOI radius is 119,243,724m, and crossing it forces `PhysicsMode = false` — an abrupt, `[UNTESTED-LIVE]` handover to Kepler rails propagation, not a smooth one; any burn/drag/rotation in progress at that instant stops being simulated. Real-time cost to physically travel that far after escape (v_inf likely small) is plausibly tens of hours at real physics tick rate, incompatible with continuous 60Hz telemetry recording. Descoped this session's actual flight to "leave the atmosphere (h>30km) + randomized arbitrary inputs (turns/throttle/RCS) throughout the vacuum arc", with the full SOI crossing deferred to its own separate, more targeted flight later.

**Mod v0.66.0 — staging (`stage <index>` / `stages`), IL-researched and live-verified.** See `sfsprobe/mod_changelog.md` for the full IL chain (`StagingDrawer.UseStage` → `Rocket.UseParts`). Live-tested end to end on Christian's real 3-stage/27-part rocket: stage 0 (4x Hawk, ignition only) → stage 1 (2x separator + Titan, separation+ignition together, partCount 27→13, mass 279.5t→145.1t) → stage 2 (final separator + Valiant, partCount 13→7, mass →47.1t). Resolved the previously-`[OPEN]` "is `stages[0]` next to fire" question empirically rather than by assumption: firing doesn't pop a stage from the list by usage, the list just reflects whichever parts are still physically attached (a stage vanishes only once ALL its parts are destroyed/detached, which is why the engine-only stage 0 disappeared — its Hawks were physically part of the boosters jettisoned in stage 1, not because "stage 0 was used").
...
**1. `integrator` flag added.** `_symplectic_euler_step()` (new function, one derivative evaluation per step, velocity-like state updated from the OLD state then position-like state updated from the NEW velocity -- the defining symplectic trait) is now a selectable alternative to `_rk4_step()` via a new `integrator: str = "rk4"` parameter, threaded through `forward_simulate`, `_prepare_replay`, `test_against_run`, and `test_against_run_trajectory`. Tested against the tagged flight (dt=1/60s, 6s window): rotation/angular-velocity error virtually identical between the two (confirms rotation genuinely doesn't care which integrator is used), but **position accuracy is 4-10x worse** with symplectic Euler (median 0.69m vs. RK4's 0.067m) -- expected, RK4 is 4th-order accurate vs. symplectic Euler's 1st-order, at the same dt. Real, quantified trade-off: symplectic Euler is faster and closer to the game's actual internal scheme, RK4 is more accurate at a given step size.

**2. `lookup_field()` speedup, 24.5x for RK4 / 12.3x for symplectic Euler.** Profiled a 10-minute (600s) forward-sim: 96% of wall-clock time was inside `aoa_dragarea.py`'s `lookup_field()`, which was re-sorting the ENTIRE bin list (721 bins) from scratch on every single call (~48k calls for just a 6s/60Hz replay) and then linear-scanning for the bracket -- both totally unnecessary since the table's bins never change between calls in any real use. Fixed: the sort now happens ONCE per (table, field) and is cached directly on the table dict itself (`table["_lookup_cache"]`), and the bracket search uses `bisect` (O(log n)) instead of a linear scan (O(n)). **Verified byte-for-byte identical output** against the original implementation across the full -180..180 range for all three fields (dragArea, dragCopX, dragCopY) before promoting -- 10,803 test points, 0 mismatches. Timing: a 10-minute forward-sim went from 3.43s to 0.14s (RK4) and 1.11s to 0.09s (symplectic Euler). Original kept as `aoa_dragarea.py.bak-pre-lookup-speedup` for the comparison.

**3. Honest longer-horizon finding, NOT a bug, NOT a regression from #2** (independently re-confirmed using the OLD, unoptimized `lookup_field` at the same settings -- identical result either way). Requesting a full 600s replay of the tagged flight only produces ~4 comparison points (stride=1000) because the simulation correctly predicts terrain impact (`collided: True`) around sim_t~50s -- consistent with the real flight's own eventual fate (fell back to pad height by its own real t~59s), just not exactly time-matched. The rotation error in that late, falling-back-down portion is substantial (median 72 deg across the handful of samples that far out) -- well outside the 6-second pure-ascent window this session's fix was actually validated against. Not alarming and not new: this is very plausibly the same still-open small-AoA instability (0.1-2 deg, found earlier this session, never addressed) becoming visible over a much longer horizon than the clean validation window covered. Flagging clearly rather than letting the 6s validation imply more than it actually showed: **the fix is confirmed correct for the ascent phase specifically; full-flight (through coast/apoapsis/descent) accuracy is still unconfirmed.**

## 2026-09-07 — ROOT CAUSE FOUND AND FIXED: rotation-blowup was a position-contamination bug in the AoA sweep table, not a formula/sign error

**The whole-session story, in order.** This picks up directly from 2026-09-06's open item (aero_torque formula validated in isolation but full closed-loop replay diverges from -0.12 to +78 in 6s). Systematically ruled out, one at a time, over many hours: RK4 vs. symplectic Euler integrator (identical divergence either way), dt=0.25s vs. the game's real 1/60s (identical), finite-differenced vs. native starting velocity (identical, though found and fixed a real script-side index-wraparound bug along the way), a zero-thrust script bug (found and fixed separately), the torque-generation formula and integration step (both confirmed structurally identical to Box2D's real public source), SAS/aero composition order (real, IL-confirmed, kept as a fix, but proven inert on this flight since `output_TurnAxisTorque` was 0 the entire time), Box2D's real per-step rotation safety clamp (added, real, but confirmed to only affect the tail once things are already wrong, not the onset), live-tracked vs. frozen inertia (real ~25% swing over a partial burn, confirmed via live `rb2d.inertia`/mass telemetry, but made the blowup slightly *worse* not better), and — critically — the AoA-heading sign convention itself, which a live `setrot`+`computed:aeroTorque` test at ±20/30/40° confirmed CORRECT (real torque genuinely restoring at those angles). A parallel live test at very small angles (0.1°-2°) found a real, separate, genuine small-AoA aerodynamic instability in this specific finless craft — real, confirmed via sustained real `rb2d.rotation` drift over a full 0.5s window, but NOT the cause of the simulated blowup (later confirmed below).

**THE ACTUAL ROOT CAUSE**, found via a direct live `setrot` + `dragarea` comparison against `forward_sim.py`'s own table lookup at the identical real (theta, velocity) state, mid-flight: `dragCopX/Y` (from `Aero_Rocket.GetDragSurfaces`, confirmed via IL — `Transform.TransformPoint`) are genuine ABSOLUTE WORLD coordinates, not small position-independent offsets from CoM as every piece of code in this project (including the mod's own AoA-sweep tooling) had always assumed. The static AoA sweep table (`dragareasweep`/`getforwardstartinfo`'s auto-table) is built by holding the craft's real position/rotation fixed at pre-ignition and rotating a SYNTHETIC heading per bin to fake different AoA values — meaning each bin's stored "CoP" is dominated by this large absolute pre-ignition position, rotated by an arbitrary synthetic angle unrelated to the craft's real current position. Live confirmation: at a real flown AoA of 9.42°, the table predicted `copX=+446.68`; the real live `dragarea` command at that exact instant gave `copX=-300.70` — opposite sign, not noise. The error is ~0 at liftoff (table position ≈ real position) and grows without bound as the craft climbs away from where it was swept — exactly matching the blowup's actual shape all session.

**THE FIX, two parts, both now in `forward_sim.py`:**
1. `build_position_independent_aoa_table()` (new function) — rebuilds each table bin's `dragCopX/Y` into the TRUE, position-independent lever arm (`cop_true_world - com_at_sweep_time`, expressed in that bin's own synthetic sweep-frame) using the sweep-time rotation (`getforwardstartinfo`'s `rotationDeg`, now also returned in `craft_config`). A plain rotation by today's real heading (the code `_aero_force_and_cop` already did) then correctly reconstructs today's real lever arm regardless of how far the craft has moved. Wired automatically into `_prepare_replay` (shared by `test_against_run`/`test_against_run_trajectory`), so both official tools get the fix with no caller changes needed.
2. Live CoM tracking (`_derivative`'s new `start_pos` parameter) — `com_local` used to be frozen for the whole simulated flight; now translated every step by exactly how far the craft's root has moved since the replay started, matching what the real game does naturally by re-reading `rb2d.worldCenterOfMass` fresh every tick (confirmed via `TryComputeAeroTorque`'s own source).

Also folded into `forward_sim.py` as part of the same pass (both confirmed real during the investigation, kept even though neither alone was the root cause): `live_inertia` flag (scales inertia with live-integrated mass, matching the confirmed real `rbInertia`/mass proportionality) and `box2d_rotation_clamp` flag (caps per-step rotation to 90°, matching Box2D's real `b2_maxRotation` safety valve). Both default True in `DEFAULT_FLAGS`. The SAS/aero composition-order fix was also promoted into `forward_simulate`'s main loop (moved the discrete SAS/manual omega correction to run BEFORE the RK4 step instead of after, per the confirmed real per-tick execution order) even though it tested as inert on this specific flight — it's a real, separate, IL-confirmed correction independent of the position bug.

**VALIDATION** — official `test_against_run_trajectory`, tagged `straight_up_predictor_test_2026-09-07` flight, `start_t=0.02s`, `duration_s=6.0`, `dt=1/60s` (the game's real physics rate), run in a fresh Python process (the long-running `sfsprobe_mcp` server needed a restart to pick up the file edit — same gotcha this project has hit before, noted again for the record):

```
position_offset_m:          median=0.067m   mean=0.099m   max=0.31m
rotation_error_deg:         median=0.00048  mean=0.0058   max=0.035
angular_velocity_error_degs: median=0.0018  mean=0.0065   max=0.033
```

Before this fix, the same replay diverged to 179°+ rotation error and clamped at ~15,400°/s angular velocity error by the end of the same window. **Rotation error is now three orders of magnitude smaller than the blowup it replaces**, and stays bounded and small the entire 6-second window rather than diverging.

**Honest scope notes:**
- The real small-AoA instability found earlier this session (real, confirmed via live `rb2d.rotation` drift at 0.1-2° AoA, likely specific to this finless capsule-forward craft shape) is a SEPARATE, genuine finding, not addressed by this fix — worth its own investigation later (crossover point between 2° and 20° not yet bracketed).
- RCS's post-step handling was NOT restructured for composition order in this pass — the validated craft/flight has zero RCS modules; an RCS-heavy craft is untested territory for that specific fix.
- `build_position_independent_aoa_table` assumes the sweep table and `com_local` were captured in the same `getforwardstartinfo` call (true for the normal/documented usage) — a table+CoM pair captured separately, or after a staging event changed the craft's real geometry, would need re-validating.
- Direct callers of `forward_simulate` that bypass `_prepare_replay` (i.e. don't go through `test_against_run`/`test_against_run_trajectory`) MUST call `build_position_independent_aoa_table` themselves before passing a raw sweep table in, or they will silently reproduce the exact bug this entry just fixed — documented in both functions' docstrings, not silently assumed obvious.

See `bookkeeping/active_state.md` and `docs/high_level_checklist.md`'s forward-integrator section for the same story in project-status form, and Mnemoverse (`project:sfs-agent`) for the individual ruled-out-candidate writeups from earlier in the session.
