...
namespace SFSProbe
{
    ...
    public static class Probe
    {
        ...
        public static readonly ProbeFieldInfo[] FieldRegistry = new ProbeFieldInfo[]
        {
            ...
            new ProbeFieldInfo { Key = "comX", Namespace = "computed", Group = "aeroTorque", Unit = "Unity small-scale local physics frame meters",
                Description = "NEW v0.65.0: real per-tick rb2d.worldCenterOfMass.x -- was already computed inside TryComputeAeroTorque, just not previously surfaced. Needed to convert dragCopX/Y (VELOCITY-ALIGNED frame, confirmed) into a valid lever arm without reusing a stale pre-ignition com_local snapshot.", RequestAs = "telemetry on ...,computed:aeroTorque" },
            new ProbeFieldInfo { Key = "comY", Namespace = "computed", Group = "aeroTorque", Unit = "Unity small-scale local physics frame meters",
                Description = "NEW v0.65.0: real per-tick rb2d.worldCenterOfMass.y -- see comX.", RequestAs = "telemetry on ...,computed:aeroTorque" },
            new ProbeFieldInfo { Key = "copAppliedX", Namespace = "computed", Group = "aeroTorque", Unit = "world-frame meters",
                Description = "NEW v0.65.0: real per-tick center-of-drag AFTER both the real localToWorld rotation (dragCopX/Y's raw value is velocity-aligned, not world-frame) and the confirmed Lerp(CoM,CoP,0.2) blend -- directly usable as cop_x for a torque calc, no further transformation needed.", RequestAs = "telemetry on ...,computed:aeroTorque" },
            new ProbeFieldInfo { Key = "copAppliedY", Namespace = "computed", Group = "aeroTorque", Unit = "world-frame meters",
                Description = "NEW v0.65.0: real per-tick center-of-drag, y component -- see copAppliedX.", RequestAs = "telemetry on ...,computed:aeroTorque" },
            ...
            new ProbeFieldInfo { Key = "hasControl", Namespace = "inputs", Unit = "bool",
                Description = "Rocket.hasControl -- NEW v0.64.0, one of SAS's two gate conditions (GetTurnAxis, sfs_source_reference.md B1.3). Needed alongside isOnSurface/outputTurnAxisTorque to distinguish real SAS auto-correction from a hands-off no-op.",
                RequestAs = "telemetry on (full mode, no field list) -- inputs.jsonl" },
            new ProbeFieldInfo { Key = "isOnSurface", Namespace = "inputs", Unit = "bool",
                Description = "Rocket.IsOnSurface -- NEW v0.64.0, SAS's other gate condition (GetTurnAxis requires hasControl && !IsOnSurface). Grounded (true) structurally blocks SAS regardless of hasControl.",
                RequestAs = "telemetry on (full mode, no field list) -- inputs.jsonl" },
            new ProbeFieldInfo { Key = "outputTurnAxisTorque", Namespace = "inputs", Unit = "-1 to 1 float, saturating",
                Description = "Rocket.output_TurnAxisTorque -- NEW v0.64.0, the REAL resolved turn-axis value ApplyTorque actually uses (manual input if nonzero, else SAS's own deadbeat correction, else 0). NOT the same as turnAxis (raw arrowkeys input only) -- confirmed live 2026-09-07 this can be fully saturated (+-1) while raw turnAxis reads exactly 0, whenever SAS has auto-engaged.",
                RequestAs = "telemetry on (full mode, no field list) -- inputs.jsonl" },
        ...
        static string BuildInputsSample(object r, object rb, double t)
        {
            ...
            // NEW v0.64.0 -- H1 SAS-engagement finding (see analysis/
            // python_changelog.md's 2026-09-07 H1 entries). Every flight this
            // project has recorded so far only ever captured raw
            // arrowkeys.turnAxis (above), which reads 0 the instant a player
            // releases the stick -- but the REAL resolved value ApplyTorque
            // actually uses, Rocket.output_TurnAxisTorque, can be fully
            // saturated (+-1) at that exact moment if SAS has auto-engaged
            // (confirmed live 2026-09-07: GetTurnAxis's manual-vs-SAS branch,
            // gated on hasControl && !IsOnSurface). Any control-schedule
            // replay built from turnAxis alone silently drops all SAS
            // activity. Recording all three together (the resolved output
            // plus its own two gate conditions) closes that gap for every
            // future flight -- same ResolvePath-equivalent access pattern
            // (GetWrapped2(Get(r, name))) already confirmed live via
            // telemetrysnapshot before this was wired in here.
            bool hasControl = ToB(GetWrapped2(Get(r, "hasControl")));
            bool isOnSurface = ToB(GetWrapped2(Get(r, "IsOnSurface")));
            float outputTurnAxisTorque = ToF(GetWrapped2(Get(r, "output_TurnAxisTorque")));
            ...
            si.Append(",\"hasControl\":").Append(hasControl ? "true" : "false");
            si.Append(",\"isOnSurface\":").Append(isOnSurface ? "true" : "false");
            si.Append(",\"outputTurnAxisTorque\":").Append(Num(outputTurnAxisTorque));
        ...
        static bool AppendComputedField(string name, object rocket, StringBuilder sb)
        {
            switch (name)
            {
                ...
                case "aeroTorque":
                {
                    ...
                    // NEW v0.65.0 -- H1-followup diagnostic (see
                    // analysis/python_changelog.md's 2026-09-07 H1
                    // entries). comXo/comYo (real per-tick
                    // rb2d.worldCenterOfMass) and copAX/copAY (the
                    // world-frame, Lerp(CoM,CoP,0.2)-APPLIED center of
                    // drag) were ALREADY computed inside
                    // TryComputeAeroTorque for the torque calc above --
                    // just not previously surfaced to telemetry. Exposes
                    // the REAL per-tick CoM the Python side needs to
                    // correctly convert dragCopX/Y (confirmed
                    // VELOCITY-ALIGNED space, see TryComputeAeroTorque's
                    // own header comment) into a lever arm without
                    // reusing a stale pre-ignition snapshot -- two
                    // earlier attempts at this diagnostic (using a
                    // frozen pre-ignition com_local, or the wrong-frame
                    // location.position.x/y) both produced spurious,
                    // exploding results for exactly this reason.
                    sb.Append(",\"comX\":").Append(ok ? Num(comXo) : "null");
                    sb.Append(",\"comY\":").Append(ok ? Num(comYo) : "null");
                    sb.Append(",\"copAppliedX\":").Append(ok ? Num(copAX) : "null");
                    sb.Append(",\"copAppliedY\":").Append(ok ? Num(copAY) : "null");
                    return true;
                }
                case "gimbal":
                {
